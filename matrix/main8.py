...
class _Matrix():
    def __init__(self, size):
        ...
        self.size = size
        for i in range(size):
            self.data.append(array.array('H', (0 for _ in range(self.size))))
    def __repr__(self):
        retval = ''
        if self.size > 40:
            return 'too big to print matrix'
        for i in range(self.size):
            retval += str(self.data[i].tolist()) + '\n'
        return retval
    def get_count(self):
        retval = 0
    ...
    def _matmul(self,_mul, _sum, _map,_print,_range,other):
        size = self.size
        retval = Matrix(size)
        for i_row in _range(size):
            now = monotonic()
            row_data = self.data[i_row]
            for i_col in _range(size):
                col_data = other.data[i_col]
                retval.data[i_row][i_col] = _sum(_map(_mul, row_data, col_data))
            _print(i_row, monotonic() - now)
        return retval
    __matmul__ = partialmethod(_matmul,operator.mul,sum,map,print,range)
...
def run(a,b,w,h):
    ...
    #cProfile.runctx('mulitply_matrix(am,h-1)',globals=globals(),locals=locals())
